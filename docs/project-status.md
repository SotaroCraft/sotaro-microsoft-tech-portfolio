...
| Static Web Apps | Bicep 定義済 — **未デプロイ** | 🟡 Phase A（要承認） |
| Azure Functions（SWA linked） | SWA 連携で自動 | 🟡 Phase A（要承認） |
...
| GitHub Actions（SWA CI/CD） | ✅ | `.github/workflows/azure-static-web-apps.yml` — **トークン未設定** |
...
|-------|------|--------|----------------|
| **A** | SWA + GitHub Actions + Cosmos 本番接続 | ✅ Bicep + workflow | ⬜ **要承認** |
...
### Phase A — 承認後のデプロイ手順（未実行）

1. ユーザー承認（SWA Free + Functions Consumption 無料枠想定）
2. `az deployment sub create` で `infra/main.bicep`（SWA モジュール含む）— **新規 RG への SWA 作成は既存 Cosmos と別デプロイの場合は `resources.bicep` の SWA モジュールのみ再適用**
3. Azure Portal で SWA ↔ GitHub リポジトリ連携
4. GitHub Secret `AZURE_STATIC_WEB_APPS_API_TOKEN` を設定
5. `main` へ push → Actions でビルド・デプロイ
6. SWA App Settings に本番 Cosmos 接続文字列を確認
